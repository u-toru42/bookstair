...
RUN bundle install && bundle lock --add-platform x86_64-linux && bundle exec rails css:install:tailwind && bundle exec rails javascript:install:esbuild && RAILS_ENV=production bundle exec rake assets:precompile
