...
RUN sudo ldconfig & bundle install && bundle lock --add-platform x86_64-linux && bundle exec rails css:install:tailwind && bundle exec rails javascript:install:esbuild
...
# Sidekiqを起動するためのコマンドを指定(redis起動も追加)
CMD redis-server & bundle exec sidekiq -e development -C config/sidekiq.yml -r ./config/boot.rb -r ./config/schedule.rb
# ジョブ実行のために必要な設定を追加する
